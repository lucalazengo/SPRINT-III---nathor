...
def create_initial_schedule(unscheduled_items, daily_capacity, setup_cor):
    """Cria a programação inicial usando a heurística de priorização."""
    schedule, day_number, remaining_items = [], 1, copy.deepcopy(unscheduled_items)
    while remaining_items:
        ...
        remaining_items = [item for item in remaining_items if item not in to_remove]
        day_number += 1
    return schedule
...
def tabu_search_optimizer(daily_sequence, config, initial_item=None):
    ...
    for _ in range(config['max_iterations']):
        ...
        if best_neighbor:
            ...
            if len(tabu_list) > config['tabu_tenure']: tabu_list.pop(0)
...
def run_full_optimization(task_list, config):
    ...
    initial_schedule = create_initial_schedule(task_list, config['daily_capacity'], config['setup_cor'])
    optimized_schedule = []
    last_item = None
